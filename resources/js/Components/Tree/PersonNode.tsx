...
/**
 * PersonNode — بطاقة الذكر في الشجرة.
 * تمييز بصري حسب الحالة (مرئي من بعيد):
 *   - حيّ:    تدرّج عسلي/كريمي + إطار ذهبي + نقطة خضراء
 *   - متوفى:  تدرّج وردي/أحمر فاتح هادئ
 *   - غير معروف: أبيض (الافتراضي)
 */
function PersonNodeComponent({ data, selected }: NodeProps & { data: PersonNodeData }) {
    ...
    if (isLiving) {
        ...
        cardBg = 'bg-gradient-to-b from-rose-200/80 via-rose-100/60 to-white';
        cardBorder = selected
            ? 'border-rose-400 shadow-[0_0_0_4px_rgba(244,114,182,0.2),0_8px_24px_rgba(0,0,0,0.15)] scale-[1.02]'
            : 'border-rose-300/80 hover:border-rose-400 hover:shadow-xl';
    } else {
        cardBg = 'bg-white';
        cardBorder = selected
            ? 'border-gold-light shadow-[0_0_0_4px_rgba(201,168,76,0.2),0_8px_24px_rgba(0,0,0,0.15)] scale-[1.02]'
            : 'border-gold/20 hover:border-gold/40 hover:shadow-xl';
    }
    ...
    return (
        <div className="relative" dir="rtl">
            <Handle
                ...
                className="!w-2 !h-2 !bg-gold !border-0 !opacity-0 !pointer-events-none"
            />

            <div
                className={`
                    relative ${cardBg} rounded-2xl shadow-lg px-5 py-4 min-w-[180px]
                    border-2 transition-all duration-200
                    ${cardBorder}
                `}
            >
                {/* نقطة "حيّ" في الزاوية العليا */}
                {isLiving && (
                    <span
                ...
                <div className="text-center">
                    ...
                    {data.title && (
                        <div className="text-gold text-[11px] mt-0.5 font-medium">
                            {data.title}
                        </div>
                    )}
                    {yearRange && (
